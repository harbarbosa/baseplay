...
<div class="card">
    <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
            <h1>Agenda</h1>
            <p style="color:var(--muted);">Eventos, convocaÃƒÂ§ÃƒÂµes e presenÃƒÂ§a.</p>
        </div>
    ...
    <div style="margin-top:12px;">
        ...
        <a href="<?= base_url('/events?view=calendar') ?>" class="button secondary">CalendÃƒÂ¡rio</a>
    </div>
    ...
    <form method="get" action="<?= base_url('/events') ?>" style="margin:16px 0; display:flex; gap:12px; flex-wrap:wrap;">
        ...
        <select name="status">
            ...
            <option value="completed" <?= ($filters['status'] ?? '') === 'completed' ? 'selected' : '' ?>>ConcluÃƒÂ­do</option>
        </select>
    ...
    <?php if ($viewMode === 'calendar'): ?>
        ...
        <div style="display:grid; grid-template-columns: repeat(7, 1fr); gap:8px;">
            <?php foreach (['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'SÃƒÂ¡b', 'Dom'] as $dayLabel): ?>

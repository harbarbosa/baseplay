...
<div class="card">
    ...
    <form method="post" action="<?= base_url('/guardians/' . $guardian['id'] . '/update') ?>">
        <?= csrf_field() ?>
        <div class="form-group">
            <label for="full_name">Nome completo</label>
            <input id="full_name" name="full_name" type="text" value="<?= esc(old('full_name') ?? $guardian['full_name']) ?>" required>
        </div>
        <div class="form-group">
            <label for="phone">Telefone</label>
            <input id="phone" name="phone" type="text" value="<?= esc(old('phone') ?? $guardian['phone']) ?>">
        </div>
        <div class="form-group">
            <label for="email">E-mail</label>
            <input id="email" name="email" type="email" value="<?= esc(old('email') ?? $guardian['email']) ?>">
        </div>
        <div class="form-group">
            <label for="relation_type">Parentesco</label>
            <input id="relation_type" name="relation_type" type="text" value="<?= esc(old('relation_type') ?? $guardian['relation_type']) ?>">
        </div>
        <div class="form-group">
            <label for="document_id">Documento</label>
            <input id="document_id" name="document_id" type="text" value="<?= esc(old('document_id') ?? $guardian['document_id']) ?>">
        </div>
        <div class="form-group">
            <label for="address">Endereço</label>
            <input id="address" name="address" type="text" value="<?= esc(old('address') ?? $guardian['address']) ?>">
        </div>

        <hr>
        <h3>Vincular a atleta (opcional)</h3>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <select id="status" name="status">
                <option value="active" <?= (old('status') ?? $guardian['status']) === 'active' ? 'selected' : '' ?>>Ativo</option>
                <option value="inactive" <?= (old('status') ?? $guardian['status']) === 'inactive' ? 'selected' : '' ?>>Inativo</option>
            </select>
        </div>

...
<div class="card">
    <div style="display:flex; justify-content:space-between; align-items:center;">
        ...
        <div>
            ...
            <?php if (has_permission('reports.view')): ?>
                <a href="<?= base_url('/reports/athlete/' . $athlete['id']) ?>" class="button secondary">Relatório do atleta</a>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div style="margin-top:20px;">
        ...
        <div style="display:grid; gap:12px; grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); margin-bottom:16px;">
            <div class="card" style="margin:0; border:1px solid var(--line);">
                <p style="margin:0 0 6px 0; color:var(--muted); font-size:12px; text-transform:uppercase; letter-spacing:.04em;">Último treino</p>
                <?php if (!empty($lastActivity['last_training'])): ?>
                    ...
                    <p style="margin:0; color:var(--muted); font-size:12px;">há <?= esc((string) $lastActivity['last_training']['days_ago']) ?> dias</p>
                <?php else: ?>
                    <p style="margin:0; color:var(--muted);">Sem registros</p>
                <?php endif; ?>
            </div>
            <div class="card" style="margin:0; border:1px solid var(--line);">
                <p style="margin:0 0 6px 0; color:var(--muted); font-size:12px; text-transform:uppercase; letter-spacing:.04em;">Último jogo</p>
                <?php if (!empty($lastActivity['last_match'])): ?>
                    ...
                    <p style="margin:0; color:var(--muted); font-size:12px;">há <?= esc((string) $lastActivity['last_match']['days_ago']) ?> dias</p>
                <?php else: ?>
                    <p style="margin:0; color:var(--muted);">Sem registros</p>
                <?php endif; ?>
            </div>
        ...
        <p>Observações internas: <?= esc($athlete['internal_notes'] ?? '-') ?></p>
        <p>Observações de saúde: <?= esc($athlete['medical_notes'] ?? '-') ?></p>
    </div>
        ...
        <?php if (has_permission('guardians.create')): ?>
            <form method="post" action="<?= base_url('/athletes/' . $athlete['id'] . '/guardians/link') ?>" style="margin-bottom:12px;">
                ...
                <div class="form-group">
                    <label for="guardian_id">Vincular responsável existente</label>
                ...
                <div class="form-group">
                    <label><input type="checkbox" name="is_primary" value="1"> Marcar como primário</label>
                </div>
                <div class="form-group">
            ...
            <form method="post" action="<?= base_url('/athletes/' . $athlete['id'] . '/guardians/create-link') ?>">
                <?= csrf_field() ?>
                <h3>Criar novo responsável e vincular</h3>
                <div class="form-group">
        ...
        <table class="table" style="margin-top:16px;">
            ...
            <?php foreach ($guardians as $guardian): ?>
                <tr>
                    ...
                    <td><?= $guardian['is_primary'] ? 'Sim'  : 'Não'  ?></td>
                    ...
                    <td>
                        <?php if (has_permission('guardians.update')): ?>
                            <form method="post" action="<?= base_url('/athlete-guardians/' . $guardian['id'] . '/update') ?>" style="display:inline-block;">
                                ...
                                <button type="submit" class="secondary"><?= $guardian['is_primary'] ? 'Desmarcar'  : 'Tornar primário'  ?></button>
                            </form>
                        <?php endif; ?>
                        <?php if (has_permission('guardians.delete')): ?>
                            <form method="post" action="<?= base_url('/athlete-guardians/' . $guardian['id'] . '/delete') ?>" style="display:inline-block;">
                                <?= csrf_field() ?>
                                <button type="submit" class="secondary">Remover vínculo</button>
                            </form>
                        <?php endif; ?>
                    </td>
                </tr>
    ...
    <div style="margin-top:24px;">
        <h2>Histórico</h2>
        <p style="color:var(--muted);">Em breve: treinos, presenças e jogos.</p>
    </div>
</div>
<?= $this->endSection() ?>

...
<div class="card" style="max-width:800px;">
    ...
    <form method="post" action="<?= base_url('/documents/' . $document['id'] . '/update') ?>" enctype="multipart/form-data">
        <?= csrf_field() ?>
        <input type="hidden" name="guardian_id" value="<?= esc(old('guardian_id', $document['guardian_id'] ?? '')) ?>">
        ...
        <div class="form-group">
            ...
            <select name="document_type_id" required>
                ...
                <?php foreach ($types as $type): ?>
                    <option value="<?= esc($type['id']) ?>" <?= old('document_type_id', $document['document_type_id']) == $type['id'] ? 'selected' : '' ?>>
                        <?= esc($type['name']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="form-group">
            ...
            <select name="team_id" id="doc_team_id">
                ...
                <?php foreach ($teams as $team): ?>
                    <option value="<?= esc($team['id']) ?>" <?= old('team_id', $document['team_id']) == $team['id'] ? 'selected' : '' ?>>
                        <?= esc($team['name']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
            ...
            <select name="athlete_id" id="doc_athlete_id" disabled>
                ...
                <?php foreach ($athletes as $athlete): ?>
                    ...
                    <option
                        ...
                        <?= old('athlete_id', $document['athlete_id']) == $athlete['id'] ? 'selected' : '' ?>
                    >
                        <?= esc($full) ?>
                    </option>
                <?php endforeach; ?>
        ...
        <div class="form-group">
            ...
            <input type="file" name="document_file" accept="application/pdf,image/jpeg,image/png">
        </div>

        <div class="form-group">
            <label>Data de emissão</label>
            <input type="date" name="issued_at" value="<?= esc(old('issued_at', $document['issued_at'])) ?>">
        </div>

        <div class="form-group">
            <label>Data de vencimento</label>
            <input type="date" name="expires_at" value="<?= esc(old('expires_at', $document['expires_at'])) ?>">
        </div>

        <div class="form-group">
            <label>Observações</label>
            <input type="text" name="notes" value="<?= esc(old('notes', $document['notes'])) ?>">
        </div>
        ...
        <div class="form-group">
            ...
            <select name="status">
                ...
                <option value="expired" <?= old('status', $document['status']) === 'expired' ? 'selected' : '' ?>>Vencido</option>
                <option value="archived" <?= old('status', $document['status']) === 'archived' ? 'selected' : '' ?>>Arquivado</option>
            </select>
        </div>

        <button type="submit">Salvar</button>
        <a href="<?= base_url('/documents/' . $document['id']) ?>" class="button secondary">Cancelar</a>
    </form>
</div>
